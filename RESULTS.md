...
**Restructured as a methodological negative (2026-09-23).** Two pre-registered analyses decided the framing. (1) `mdm_trap_control` put the field's standard pipeline (covariance → MDM, pyRiemann) through the four traps that apply to classification. **None applied** at its pre-registered bar: accuracy dropped by 0.007 without EOG; two same-state recordings were told apart at 0.65 (bar 0.70; eyes open vs closed 0.87); the subject-leakage gap was 0.001; shuffled minus blocked CV was 0.009. By the pre-registered rule, these are traps of *this method*. The control changed the design as well as the classifier, though: it kept each epoch's amplitude, which this method's trace normalization removes, and used non-overlapping epochs, while this method's dependence trap arose in overlapping windows. The paper therefore reads them, as labelled interpretation, as traps of this *design*: trace normalization, overlapping windows, centred localization windows (added 2026-09-24 after review). The lesson was then made precise after a second review. Band-pass filtering followed by trace normalization removes the amplitude of a band relative to the rest of the spectrum, not "power" in general. The scalar that beat the geometry on eyes open/closed is itself normalized: relative alpha, 8–13 over 1–40 Hz, on the unfiltered signal. (2) `trap_literature_survey` coded 20 published covariance/Riemannian EEG studies. It measures reporting, not the presence of a trap. Ocular handling was unreported in 7/19, or up to 15/19 counting the unclear codes; dependence-blind validation in 4/19 (up to 12/19); the recording confound in 1/17 (up to 3/17); pseudo-replication in 0/9 (up to 4/9); and one study measured the dependence trap for the standard classifier at up to 12.7 %. The paper is now the record of the five traps. The sections below are the history of how each was found. Paper 3 section numbers in historical entries (§2 regimes, §3.2 fibre, §3.3 Sasaki metric, §7.6) refer to the earlier protocol text, preserved as `drafts/paper3_protocol_full_2026-09-23.md`; the current Paper 3 has a different structure.
