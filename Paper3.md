...
**The standard-pipeline control (`mdm_trap_control`).** To separate traps of this method from traps of the field, the field's standard classifier was put through the same controls. That classifier is covariance estimation with OAS shrinkage (Chen et al. 2010) followed by minimum distance to Riemannian mean (Barachant et al. 2012), as implemented in pyRiemann 0.12 (Barachant et al., pyRiemann). The criteria and the framing rule were fixed before the run (§4.6). The control changes the design as well as the classifier. It uses pyRiemann's unnormalized covariance, so absolute power is kept, and non-overlapping epochs (the 30 s scoring epochs for sleep, 2 s epochs for the motor/imagery corpus). This method used trace-normalized covariances of overlapping windows (1 s with a 0.25 s step; 2 s with a 1 s step for sleep). The comparison therefore separates this design from the standard one, not this classifier from another (§4.6).
...
**Standard pipeline.** Removing the EOG channel from within-record MDM classification of N2 versus REM lowered the median subject-level accuracy by only $0.007$, below the pre-registered $0.05$. The standard classifier on this task did not depend on the eye channel. *Interpretation, not a tested result:* the likely reason is that the standard covariance is not trace-normalized. N2 and REM differ in EEG power, and a classifier that keeps power can separate them without the eye channel. This method divided power out by construction. That left the correlation structure, where the EOG channel dominates, to carry the signal. The same pattern appears in §4.5, where per-channel alpha power separated eyes open from eyes closed more strongly than the geometry did.
...
| Ocular contamination | $V$ $0.452 \to 0.021$ without EOG | EOG ablation | does not apply (drop 0.007; power kept) | $7/19$ ($15/19$) |
...
- **Power.** Its covariance keeps absolute power; this method divides it out. When the states differ mainly in power, as N2 and REM, or eyes open and closed, appear to, removing power removes the dominant signal. What remains is correlation structure, which is where artefacts such as the EOG channel carry the most weight.
...
The reading the evidence supports is therefore traps of this *design* — trace normalization, overlapping windows and centred localization windows — rather than of this classifier. This is also the lesson that generalizes. Anyone who normalizes covariances to remove power should expect the method to answer to whatever the power was masking, and should test for it. Whether the standard classifier would fall into the traps on trace-normalized covariances of overlapping windows was not tested (§7).
...
A trace-normalized SPD geometry read through a geodesic change-point statistic looked, at different times, like a structural-regime discriminator, a sleep-staging tracker and an on-line localizer. Each appearance was produced by one of five ordinary traps, and each was removed by a pre-registered control. The field's standard Riemannian pipeline, run through the same controls on the same data, did not pass the bar on any of the four that apply to it. But it kept the power this method discarded and used non-overlapping epochs, so the traps are best read as traps of this design, and its $0.65$ on two same-state recordings is below the bar but above chance. Published studies often do not report enough to rule two of the traps out, which is reason enough to make the controls routine. The contribution is the record: what each trap did, the control that exposed it, and the numbers before and after.
